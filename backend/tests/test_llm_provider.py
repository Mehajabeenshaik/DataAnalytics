import os
# Respect an externally-provided provider (CI sets LLM_PROVIDER=mock).
# Only default to ollama for local manual runs.
...
# --- Manual demo script (NOT part of pytest collection) ---
# This block runs only when executed directly (`python test_llm_provider.py`)
# with LLM_PROVIDER=ollama. Under pytest (PYTEST_CURRENT_TEST is unset at
# import time, but collection imports the module) and under CI
# (LLM_PROVIDER=mock) it must NOT assert — otherwise collection fails.
if __name__ == "__main__" and os.environ.get("LLM_PROVIDER") == "ollama":
